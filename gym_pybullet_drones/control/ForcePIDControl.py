...
class ForcePIDControl(BaseControl):
    ...
    def computeControl(self,
        ...
        thrust, computed_target_rpy = self._dslPIDPositionControl(control_timestep,
                                                                         cur_quat,
                                                                         cur_vel,
                                                                         cur_accel,
                                                                         target_force,
    ...
    def _dslPIDPositionControl(self,
        ...
        accel_e = target_accel - np.squeeze(cur_accel)
        self.integral_vel_e = self.integral_vel_e + vel_e*control_timestep
        self.integral_vel_e = np.clip(self.integral_vel_e, -2., 2.)             # TODO: Tune this
        self.integral_vel_e[2] = np.clip(self.integral_vel_e[2], -0.15, .15)    # TODO: Tune this
        #### PID target thrust #####################################
        target_thrust = np.multiply(self.P_COEFF_FOR, vel_e) \
                        + np.multiply(self.I_COEFF_FOR, self.integral_vel_e) \
                        + np.multiply(self.D_COEFF_FOR, accel_e) + np.array([0, 0, self.GRAVITY]) + target_force
        # target_thrust = np.multiply(self.P_COEFF_FOR, pos_e) \
        #                 + np.multiply(self.I_COEFF_FOR, self.integral_pos_e) \
        #                 + np.multiply(self.D_COEFF_FOR, vel_e) + np.array([0, 0, self.GRAVITY])  
        #      
        scalar_thrust = max(0., np.dot(target_thrust, cur_rotation[:,2]))
        # TODO: Check output of controller against reaction force of joint---should be the same            
        scalar_thrust = max(0., np.dot(target_thrust, cur_rotation[:,2]))
